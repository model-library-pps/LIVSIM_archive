...
%Little growth for growing animals prot deficient diet
%version Updated May 2009 ---Mariana Rufino
%--------------------------------------------------------------------------

function [COW]=Little_growth_prot(COW,COW_OLD,PARAM)
...
while abs(COW.INPUT.MP-NeedsProt)>tolerance
    ...
    if COW.CHAR.Age<=PARAM.SYS.timestep
        COW.BW.average_BW =PARAM.REPR.CBW+growth_new*0.5; %actual weight minus CBW
    else
    ...
    NeedsProt      =(PARAM.AFRC.MPm+PARAM.AFRC.MPf)/1000*PARAM.SYS.YLEN*PARAM.SYS.timestep;
...
%Update balance
% 
% BALANCE.ME=COW.INPUT.ME/(COW.NEED.MaintME+COW.NEED.GrowthME);
% 
% if BALANCE.ME<1
%     need_to_go_to_LG=1;
%     Little_growth
